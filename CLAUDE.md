...
This is a **Next.js 16.2.4** app using the **App Router** with **React 19.2.4** and **Tailwind CSS v4**.
...
```
app/
  layout.tsx      # Root layout — Geist fonts, sets html/body baseline classes
  page.tsx        # Home route
  globals.css     # Tailwind v4 imports + CSS custom properties for light/dark theming
public/           # Static assets served at /
next.config.ts    # Minimal config, extend here
```
...
`@/*` maps to the project root (`tsconfig.json`), so `import Foo from '@/components/Foo'` works from anywhere.
